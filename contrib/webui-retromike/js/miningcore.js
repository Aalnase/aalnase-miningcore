...
var WebURL         = "https://test.go-poolmining.com";  // Website URL
...
//}
var API            = "https://test.go-poolmining.com/api/";  // API address
...
var stratumAddress = "test.go-poolmining.com";  // Stratum address
...
function loadIndex() {
  ...
  var supportRequested = currentPage === "support";
  if (supportRequested) {
    currentPage = "faq";
  }
      ...
      if (data.length > 0) {
        $.each(data, function(index, value) {
          ...
          //minerList +=   "<td>" + value.miner + "</td>";
			  var minerShort = value.miner.substring(0, 12) + ' &hellip; ' + value.miner.substring(value.miner.length - 12);
			  minerList +=   '<td><a class="miner-dashboard-link" title="Open miner dashboard" href="#' + currentPool + '/dashboard?address=' + encodeURIComponent(value.miner) + '">' + minerShort + '</a></td>';
...
// Format API UTC timestamps in the pool display timezone. Miningcore returns ISO UTC (Z);
// the WebUI should show a compact date + time instead of a raw JS Date string.
function formatPoolDateTime(value) {
  if (!value) return "n/a";
...
function loadBlocksPage() {
  return $.ajax(API + "pools/" + currentPool + "/blocks?page=0&pageSize=100")
    .done(function(data) {
      ...
      if (data.length > 0) {
        $.each(data, function(index, value) {
		  var createDate = formatPoolDateTime(value.created);
          ...
          blockList += "<tr>";
          blockList += '<td class="block-found-time">' + createDate + '</td>';
    ...
    .done(function(data) {
      ...
      if (data.length > 0) {
        $.each(data, function(index, value) {
          var createDate = formatPoolDateTime(value.created);
          paymentList += '<tr>';
          paymentList +=   '<td class="block-found-time">' + createDate + '</td>';
...
// Modern slide-in support panel. Replaces the obsolete standalone support page.
function showSupportSlidein() {
  $('#supportSlidein, #supportSlideinBackdrop').addClass('is-open').attr('aria-hidden', 'false');
}

function hideSupportSlidein() {
  $('#supportSlidein, #supportSlideinBackdrop').removeClass('is-open').attr('aria-hidden', 'true');
}

$(document).on('click', '#supportSlideinOpen', function(e) {
  e.preventDefault();
  showSupportSlidein();
});

$(document).on('click', '#supportSlideinClose, #supportSlideinBackdrop', function(e) {
  e.preventDefault();
  hideSupportSlidein();
});

$(document).on('keydown', function(e) {
  if (e.key === 'Escape') hideSupportSlidein();
});
